...
/* global variables */
T_SBC_PARAMS sbc_enc_params;
static TSBCData SBCData;
static TSampleData sampleData[SBC_MAX_CHANNELS +
                              1][SBC_MAX_SUBBANDS]; /* one additional line for terminator  */
//static TSampleData jointSampleData[SBC_MAX_CHANNELS + 1][SBC_MAX_SUBBANDS];
static int x[SBC_MAX_CHANNELS][(SBC_MAX_SUBBANDS + 1) * 11];
static int y[SBC_MAX_CHANNELS][SBC_MAX_SUBBANDS * 2];
...
static void createSubbandSamples4(int *pMatrix, const int *pFactors, int channels, int *pSamples,
                                  TSampleData *pSampleData);
static void calcPartials8(int *pAudio, const int *pFactors, int channels, int *pMatrix);
static void createSubbandSamples8(int *pMatrix, const int *pFactors, int channels, int *pSamples,
                                  TSampleData *pSampleData);
static void joinSubbands4(int *pSamples, TSBCData *pSBCData, TSampleData *pSampleData,
                          unsigned char *pJoint);
static void joinSubbands8(int *pSamples, TSBCData *pSBCData, TSampleData *pSampleData,
                          unsigned char *pJoint);

static void createCRC(unsigned char *pOutput);

static void computeBits(void);
static void computeSlicesInPoolStereo(int max_bitneed,
...
/**/
/************************************************************************/
int sbc_encode(unsigned char *pInputBuffer, int inputSize, T_SBC_PARAMS *params,
               unsigned char *pOutputBuffer, int *pOutputSize)
{
    int blk;
    ...
    /* create subband samples */
    {
        ...
        for (blk = pSBCData->nrof_blocks; blk > 0; blk--)
        {
            ...
            if (pSBCData->nrof_subbands == 4)
            {
                ...
                createSubbandSamples4(&y[0][0], &mik4[0][0], pSBCData->nrof_channels, (int *) pSamples,
                                      &sampleData[0][0]);
            }
            else
            {
                calcPartials8(pxLeft, &c8[0], pSBCData->nrof_channels, &y[0][0]);
                createSubbandSamples8(&y[0][0], &mik8[0][0], pSBCData->nrof_channels, (int *) pSamples,
                                      &sampleData[0][0]);
            } /* if subbands */
    ...
    /* calculate scale factors */
    {
        ...
        for (i = pSBCData->nrof_channels * pSBCData->nrof_subbands - 1; i >= 0; i--)
        {
            ...
            if (mx > 255) if (mx > 4095) if (mx > 16383) if (mx > 32767) { sf = 15; }
                        else { sf = 14; }
                    else if (mx > 8191) { sf = 13; }
                    else { sf = 12; }
                else if (mx > 1023) if (mx > 2047) { sf = 11; }
                    else { sf = 10; }
                else if (mx > 511) { sf = 9; }
                else { sf = 8; }
            else if (mx > 15) if (mx > 63) if (mx > 127) { sf = 7; }
                    else { sf = 6; }
                else if (mx > 31) { sf = 5; }
                else { sf = 4; }
            else if (mx > 3) if (mx > 7) { sf = 3; }
                else { sf = 2; }
            else if (mx > 1) { sf = 1; }
            else { sf = 0; }
    ...
    /* quantize, normalize and write subband samples */
    {
        ...
        for (;;)
        {
            ...
            if ((bits = (*pSampleData++).bits) != 0)
            {
                /* break on terminator */
                if (bits <
                    0)  /* arm assembler optimization; saves 3 cycles compared to 'if (bits == SBC_TERMINATOR)' ! */
                {
                    break;
                }
                pSampleData[-1].calc1 = ((1 << bits) - 1) << (GUARD_QUANT -
                                                              1);  /* levels including factor 1/2, guarded */
                pSampleData[-1].calc2 = (pSampleData[-1].calc1) >> (pSampleData[-1].scale_factor +
                                                                    1); /* levels/scale factors, guarded via calc1 */
            }
        }
        ...
        for (blk = pSBCData->nrof_blocks; blk > 0; blk--)
        {
            ...
            for (;;)
            {
                ...
                if ((bits = (*pSampleData++).bits) != 0)
                {
                    ...
                    qs = (*pSamples++ * pSampleData[-1].calc2 + pSampleData[-1].calc1) >>
                         (GUARD_QUANT); /* un-guard (= round towards minus infinity) */
                    /* qs is a 32 bit integer holding the data to be written within the [bits] LSBs */

#if defined F_SBC_ENC_USE_FLOAT
                    {
                        double should;
                        should = ((((double) pSamples[-1]) / (1 << (pSampleData[-1].scale_factor + 1)) + 1) * ((
                                      1 << pSampleData[-1].bits) - 1.0) - 1.0) / 2.0;
                        if (should > 0)
                        {
                            should += 0.5;
                        }
