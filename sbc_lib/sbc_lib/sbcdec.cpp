...
*
*/
int sbc_decode(unsigned char *pInputBuffer, int inputSize, unsigned char *pOutputBuffer,
               int *pOutputSize)
{
    unsigned ch;
    unsigned blk;
...

    {
        ...
        /* set terminator */
        pd[nrof_samples].bits =
            SBC_TERMINATOR; /* terminator cannot be 1 because legal bit lengths of 1 may occur ! */
        ...
        for (blk = pSBCData->nrof_blocks; blk > 0; blk--)
        {
            ...
            for (;;)
            {
                ...
                if ((bits = (*pd++).bits) != 0)
                {
                    ...
                    pd[-1].sb_sample = ((qs - (msk >> 1)) * divTable[bits]) >> (GUARD_MINUS_2 + pd[-1].bits -
                                                                                pd[-1].scale_factor);
                    if (pd[-1].sb_sample < 0)
                    {
                        pd[-1].sb_sample++;
            ...
            /* reconstruct audio samples */
            {
                ...
                if (pSBCData->nrof_subbands == 4)
                {
                    ...
                    calcAudioSamples4(pv, d4, (short *) pOut, pSBCData->channel_mode == SBC_MODE_MONO,
                                      pSBCData->nrof_channels);
                }
                else
                {
                    processMatrixing8(&nik8[0][0], &sampleData[0][0].sb_sample, pv, pSBCData->nrof_channels);
                    calcAudioSamples8(pv, d8, (short *) pOut, 2, pSBCData->channel_mode == SBC_MODE_MONO,
                                      pSBCData->nrof_channels);
                } /* if 4 subbands */
...
static void initVTab(int *v)
{
    ...
    for (i = 0; i < 20; i++)
    {
        ...
        v[10 * i + 5 + 4] = (int) &v[10 * matrixingTable[i] +
                                     5]; /**< set pointer to successor group, second 4 */
    }
}
...
/**/
/************************************************************************/
int sbc_get_params(unsigned char *pInputBuffer, int inputSize, T_SBC_PARAMS *pParams)
{
    unsigned int c;
    unsigned char *pIn = pInputBuffer;
    ...
    pParams->blockNumber = blockNumbers[(c >> 4) & 0x03];
    pParams->channelMode = (c >> 2) & 0x03;
    pParams->allocMethod = (c >> 1) & 0x01;
    pParams->subbandNumber = subbandNumbers[(c >> 0) & 0x01];
    pParams->bitpool = *pIn++;
